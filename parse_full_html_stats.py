...
source_map = {}
for name, src in officer_matches:
...
for o in target_officers:
    ...
    # 判斷是否為真實核對資料 (在 updated_declarations 中或出處非預設樣板)
...
out.append(f"**總採計官員與民代數**：**{tot_all} 位**")
...
for county in sorted(by_county.keys()):
    ...
    note = "六都/監察院專刊涵蓋" if pct >= 50 else "非直轄市/部分現場查閱"
    out.append(f"| **{county}** | **{r} 位** | {d} 位 | {tot} 位 | **{pct:.1f}%** | {note} |")

with open("report.md", "w", encoding="utf-8") as f:
    f.write("\n".join(out))

print("SUCCESS! Generated report.md with precise categories.")
